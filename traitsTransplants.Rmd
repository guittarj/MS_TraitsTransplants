...
trait.data <- trait.data.full %>%
  select(speciescode, leaf.area, max.height, seed.mass, sla, conper, offs, lat, buds) %>%
...
# Plot
fig2 <- ggplot(x, aes(x = value, y = cwm, weight = len, group = 1, fill = factor(precip.level), 
    shape = factor(temp.level))) +
  geom_point() +
  ...
  stat_smooth(method = 'lm', se = FALSE, color = 'black', 
    data = filter(x, paste(trait, variable) %in% 
      paste(tats$trait, tats$variable)[tats$trait != 'conper'])) + 
  labs(x = '', y = '') +
  theme_bw() +
  theme(
